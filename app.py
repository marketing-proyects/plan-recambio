...
def load_prices():
    ...
    # Limpiar nombres para facilitar el cruce
...
# --- PESTAÑA 2: CATÁLOGO ---
    ...
    # Mapeo actualizado basado en tu Excel
    nombres_reales = {
    ...
    if os.path.exists(p):
        ...
        if archivos:
            ...
            nombre_busqueda = mostrar_nombre(sel).upper()
            row = df_precios[df_precios['Producto_Clean'].str.upper() == nombre_busqueda]
            precio_unit = float(row['Precio'].iloc[0]) if not row.empty else 0.0

            ci, cs = st.columns(2)
            with ci: st.image(os.path.join(p, sel), width=280)
            with cs:
                st.subheader(f"Precio Lista: ${precio_unit:,.2f}")
                ...
                if st.session_state.dto_base < 20:
                    ...
                    if num_en_carro >= 2: # Al añadir el siguiente será el 3ero
                        st.success("¡Esta unidad activará el 30% en todo el pedido!")
                        dto_item = 30
                    else:
                        dto_item = 20
...
# --- PESTAÑA 3: PEDIDO ---
    ...
    if st.session_state.carrito:
        ...
        for i, item in enumerate(st.session_state.carrito):
            ...
            cb.write(f"Price: ${item['precio']:,.2f}")
            cc.write(f"**-{item['dto']}%**")
        ...
        def generate_pdf():
            ...
            pdf.cell(200, 10, txt="RESUMEN DE PEDIDO - PLAN RECAMBIO", ln=True, align='C')
            pdf.set_font("Arial", '', 12)
            pdf.ln(10)
            pdf.cell(200, 10, txt=f"Cliente: {st.session_state.nombre_cliente} (N° {st.session_state.numero_cliente})", ln=True)
            pdf.ln(5)
            ...
            pdf.cell(40, 10, "Precio Final", 1, ln=True)
            
            pdf.set_font("Arial", '', 9)
            for item in st.session_state.carrito:
                p_final = item['precio'] * (1 - item['dto']/100)
                pdf.cell(100, 10, item['prod'][:50], 1)
                pdf.cell(30, 10, f"${item['precio']:,.2f}", 1)
                pdf.cell(20, 10, f"{item['dto']}%", 1)
                pdf.cell(40, 10, f"${p_final:,.2f}", 1, ln=True)
            
            pdf.ln(10)
            pdf.set_font("Arial", 'B', 12)
            pdf.cell(200, 10, txt=f"TOTAL FINAL: ${total_pagar:,.2f}", ln=True, align='R')
