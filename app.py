...
# --- PESTAÑA 3: PEDIDO ---
    ...
    if st.session_state.carrito:
        ...
        def generate_pdf():
            ...
            precio_lista_original = 0
            for it in st.session_state.carrito:
                sb = it['precio'] * (1 - it['dto']/100)
                precio_lista_original += it['precio']
            ...
            pdf.ln(10)
            ahorro_calculado = precio_lista_original - total_acumulado
            
            pdf.set_font("Arial", 'B', 12)
            pdf.cell(190, 10, f"TOTAL: ${total_acumulado:,.2f}", ln=True, align='R')
            
            # Recuadro de ahorro en rojo (solo para el PDF)
            pdf.ln(2)
            pdf.set_text_color(204, 0, 0)
            pdf.set_draw_color(204, 0, 0)
            pdf.set_line_width(0.6)
            pdf.cell(120, 10, "", 0, 0)
            pdf.cell(70, 12, f"AHORRO TOTAL: ${ahorro_calculado:,.2f}", 1, 1, 'C')
