...
# --- CARGA DE DATOS (NUEVO AGREGADO) ---
@st.cache_data
def load_data():
    try:
        # Cargamos el archivo csv que subiste
...
def buscar_info_producto(nombre_app, df):
    ...
    if df.empty: return 0.0, "Sin descripción"
    palabras_app = set(nombre_app.upper().replace('\n', ' ').split())
    
    for _, fila in df.iterrows():
        nombre_excel = str(fila['Producto']).upper().replace('\n', ' ')
        ...
        if len(coincidencias) / len(palabras_excel) >= 0.8:
            return float(fila['Precio']), str(fila['Características'])
...
# --- PESTAÑA 2: CATÁLOGO ---
    ...
    if os.path.exists(p):
        ...
        if archivos:
            ...
            # NUEVO: Búsqueda de info
            precio, caracts = buscar_info_producto(mostrar_nombre(sel), df_precios)
            ...
            with cs:
                ...
                if st.button("AÑADIR AL PEDIDO", use_container_width=True):
                    # Guardamos el precio en el carrito
...
# --- PESTAÑA 3: PEDIDO ---
    ...
    if st.session_state.carrito:
        ...
        for i, item in enumerate(st.session_state.carrito):
            ...
            p_base = item.get('precio_base', 0.0)
            p_final = p_base * (1 - item['dto'] / 100)
            total_final += p_final
            
            ca.write(f"**{i+1}.** {item['prod']}")
            cb.write(f"${p_base:,.2f}")
            cc.write(f"**-{item['dto']}%**")
        ...
        st.divider()
        st.markdown(f"### Monto Total a Pagar: ${total_final:,.2f}")
        ...
        def generate_pdf():
            ...
            for it in st.session_state.carrito:
                pb = it.get('precio_base', 0.0)
                sub = pb * (1 - it['dto']/100)
                pdf.cell(100, 10, it['prod'][:45], 1)
                pdf.cell(30, 10, f"${pb:,.2f}", 1)
                pdf.cell(20, 10, f"{it['dto']}%", 1)
                pdf.cell(40, 10, f"${sub:,.2f}", 1, ln=True)
            
            pdf.ln(5)
            pdf.set_font("Arial", 'B', 12)
            pdf.cell(190, 10, f"TOTAL FINAL: ${total_final:,.2f}", ln=True, align='R')
